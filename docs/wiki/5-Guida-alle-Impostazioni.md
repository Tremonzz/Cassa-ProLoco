...
Dalla schermata iniziale è possibile accedere al pannello completo delle **Impostazioni**.
...
* **Personalizzazione Layout**:
  Permette di personalizzare a piacimento lo scontrino da stampare. Cliccando sul tasto **`Personalizza Scontrino`** si aprirà una finestra dove è possibile configurare i seguenti campi:
...
* **Modalità Test**:
  * Quando viene stampato un ordine, al posto che mandarlo in stampa sulla stampante, viene mostrata un'anteprima digitale a video.

* **Importa/Esporta Eventi**:
  * Possibilità di esportare o di importare Eventi completi di Menu e Storico.
  * **Esporta Eventi**: Selezionando gli eventi desiderati verrà generato un file con estensione `.db` che potrà essere importato in un altro PC o conservato come copia di backup.
  * **Importa Eventi**: Selezionando un file `.db` convalidato sarà possibile scegliere quali eventi contenuti nel file importare.
